...
describe("VNode", () => {
  describe("$t", () => {
    it("init", () => {
      const t = h.t("abc");
      expect(t._flags & VNodeFlags.Text).toBeEqual(VNodeFlags.Text);
      expect(t._children).toBeEqual("abc");
    });

    it("key", () => {
      const t = h.t("abc").key("k");
      expect(t._flags & VNodeFlags.Key).toBeEqual(VNodeFlags.Key);
      expect(t._key).toBeEqual("k");
    });
  ...
  describe("$h", () => {
    it("init", () => {
      const e = h.div();
      expect(e._flags & VNodeFlags.Element).toBeEqual(VNodeFlags.Element);
      expect(e._tag).toBeEqual("div");
    });
    ...
    it("init className", () => {
      ...
      expect(e._flags & VNodeFlags.Element).toBeEqual(VNodeFlags.Element);
      expect(e._tag).toBeEqual("div");
      expect(e._className).toBeEqual("cls");
    });

    it("key", () => {
      const e = h.div().key("k");
      expect(e._key).toBeEqual("k");
    });
    ...
    it("style", () => {
      ...
      expect((e._props as ElementProps<any>).style).toBeEqual(s);
    });

    it("events", () => {
      const s = [] as EventHandler[];
      const e = h.div().events(s);
      expect((e._props as ElementProps<any>).events).toBeEqual(s);
    });

    it("props", () => {
      const s = {};
      const e = h.div().props(s);
      expect((e._props as ElementProps<any>).attrs).toBeEqual(s);
    });

    it("children", () => {
      const e = h.div().children("abc");
      expect(e._children).toBeEqual("abc");
    });
    ...
    it("mergeProps: null", () => {
      ...
      expect((e._props as ElementProps<any>).attrs!.title).toBeEqual("abc");
    });

    it("mergeProps", () => {
      const e = h.div().props({ title: "abc" }).mergeProps({ width: "100" });
      expect((e._props as ElementProps<any>).attrs!.title).toBeEqual("abc");
      expect((e._props as ElementProps<any>).attrs!.width).toBeEqual("100");
    });

    it("mergeProps override", () => {
      const e = h.div().props({ title: "abc" }).mergeProps({ title: "100" });
      expect((e._props as ElementProps<any>).attrs!.title).toBeEqual("100");
    });
    ...
    it("mergeStyle: null", () => {
      ...
      expect((e._props as ElementProps<any>).style!.top).toBeEqual("10px");
    });

    it("mergeStyle", () => {
      const e = h.div().style({ top: "10px" }).mergeStyle({ left: "20px" });
      expect((e._props as ElementProps<any>).style!.top).toBeEqual("10px");
      expect((e._props as ElementProps<any>).style!.left).toBeEqual("20px");
    });

    it("mergeStyle override", () => {
      const e = h.div().style({ top: "10px" }).mergeStyle({ top: "20px" });
      expect((e._props as ElementProps<any>).style!.top).toBeEqual("20px");
    });

    it("autofocus", () => {
      const e = h.div().autofocus(true);
      expect(e._flags & VNodeFlags.Autofocus).toBeEqual(VNodeFlags.Autofocus);
    });
  });
  ...
  describe("$i", () => {
    ...
    it("checkbox: checked", () => {
      ...
      expect(e._children).toBeEqual(true);
    });
  });
